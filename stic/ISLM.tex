...
\begin{scope}[rotate=30]
 ...
 \draw (0:\R)
     \foreach \x in {60,120,...,360} {  -- (\x:\R) }
              ...
              -- cycle (120:\R)
              -- cycle (60:\R)
              -- cycle (0:\R) [line width=4.2pt,color=lmcolor,fill=white,fill opacity=0.1];
\end{scope}

\begin{scope}[xshift=-4cm,yshift=-3.5cm,scale=0.9]
% picture with money
\node[inner sep=0pt] (russell) at (5.6,4.7){\includegraphics[angle=-190,scale=0.165]{mmm.jpg}};
...
%\draw[color=black] (-3,4) node[right] {{\color{lmcolor} {\Large \fontspec{Comic Sans MS}{LM крутится -  } }}};
%\draw[color=black] (-1,3.2) node[right] {{\color{lmcolor} {\Large \fontspec{Comic Sans MS}{лавэха мутится} }}};
\end{tikzpicture}
